...
export const ImportExcelOrganism = ({ user, onImportSuccess }) => {
  ...
  const detectCategory = (description) => {
    if (!description)
      return { name: "Lainnya", auto: false, isTransfer: false };
    ...
    const isOutgoing =
      upperDesc.includes(" KE ") ||
      upperDesc.includes(" KE:") ||
      upperDesc.includes(" TO ") ||
      upperDesc.includes(" PAYMENT ") ||
      upperDesc.includes(" PEMBAYARAN ") ||
      upperDesc.includes(" QRIS ");
    const isIncoming =
      upperDesc.includes(" DARI ") ||
      upperDesc.includes(" DARI:") ||
      upperDesc.includes(" FROM ") ||
      upperDesc.includes(" MASUK ") ||
      upperDesc.includes(" BUNGA ") ||
      upperDesc.includes(" KREDITUR ") ||
      upperDesc.includes(" PB DARI ") ||
    ...
    if (otherAccount) {
      const isTransferContext =
        ...
        upperDesc.includes("TF") ||
        upperDesc.includes("TRANSFER") ||
        upperDesc.includes("TRSF") ||
        upperDesc.includes("PB DARI");
      const myName = user?.full_name?.toUpperCase() || "";
      const hasMyName =
        (myName && upperDesc.includes(myName)) ||
        PERSONAL_NAMES.some((n) => upperDesc.includes(n));
  ...
  const isDuplicate = (date, amount) => {
    return existingTransactions.some((tx) => {
      ...
      return (
        txDate === date &&
        Math.abs(parseFloat(tx.amount)) === Math.abs(parseFloat(amount))
      );
    });
  };
  ...
  const handlePdfUpload = async (file) => {
    ...
    try {
      ...
      if (error.name === "PasswordException") {
        showNotification(
          "PDF diproteksi password! Sistem tidak bisa membukanya langsung. Silakan gunakan cara 'Tempel Teks Manual' di bawah.",
          "warning",
        );
        setShowRawPaste(true);
      } else {
        showNotification(
          "Gagal membaca file PDF. Silakan coba mode Tempel Teks.",
          "error",
        );
      }
    } finally {
  ...
  const processRawText = (text) => {
    try {
      ...
      if (isBSITarget || (!isBCATarget && !isBNITarget && !isPermataTarget)) {
        const bsiPattern =
          /(\d{1,2})\s+([A-Z,a-z]{3,})\s+(\d{4})\s*[ \d:]{0,10}?\s*(.+?)\s+(Dana Keluar|Dana Masuk|Biaya)\s*[|]?\s*(.+?)\s+([\d.,]+)\s+([\d.,]+)\s+([\d.,]+)/gi;
        while ((match = bsiPattern.exec(cleanText)) !== null) {
          ...
          const type =
            dir.includes("Masuk") || credit > 0 ? "income" : "expense";
          const fullDesc = `${prefixDesc.trim()} ${desc.trim()}`.replace(
            /\s+/g,
            " ",
          );
          results.push(
      ...
      if (isBNITarget || (!isBSITarget && !isBCATarget && !isPermataTarget)) {
        ...
        while ((match = bniPatternLocal.exec(cleanText)) !== null) {
          ...
          const type =
            signUpper.startsWith("C") || signUpper === "O1"
              ? "income"
              : "expense";
          const result = createMappedItem(
            index++,
            date,
            desc.trim(),
            amount,
      ...
      if (isPermataTarget || (!isBSITarget && !isBCATarget && !isBNITarget)) {
        const permataPatternLocal =
          /(\d{1,2})\s+([A-Z,a-z]{3,})\s+(\d{4})\s+(.+?)\s+Rp\s+([\d,.]+)/gi;
        while ((match = permataPatternLocal.exec(cleanText)) !== null) {
          ...
          let type = "expense";
          if (
            descUpper.includes("MASUK") ||
            descUpper.includes("DARI") ||
            descUpper.includes("BUNGA") ||
            descUpper.includes("PB DARI") ||
            descUpper.includes("KREDITUR")
          ) {
            type = "income";
          }
          results.push(
      ...
      if (isBCATarget || (!isBSITarget && !isBNITarget && !isPermataTarget)) {
        const bcaMobilePatternLocal =
          /(\d{1,2}\/\d{1,2})\s+(.+?)\s+([\d,.]+)\s+(DB|CR)/gi;
        ...
        const bcaPatternLocal =
          /(\d{1,2}[\/\d]{3,5})\s+([A-Z0-9.\/ ]{3,})?\s*([CDO][BR18])\s+([\d,.]+)/gi;
        while ((match = bcaPatternLocal.exec(cleanText)) !== null) {
          const [_, dateStr, desc, sign, amtStr] = match;
          const dPart = dateStr.includes("/")
            ? dateStr.split("/")[0]
            : dateStr.match(/^\d{1,2}/)[0];
          const mPart = dateStr.includes("/")
            ? dateStr.split("/")[1]
            : dateStr.match(/\d{1,2}$/)[0];
          const date = `${year}-${mPart.padStart(2, "0")}-${dPart.padStart(2, "0")}`;
          const amount = sanitizeAmount(amtStr);
      ...
      const appPatternLocal =
        /(\d{1,2})\s+([A-Z,a-z]{3})\s+(\d{4})(?:\s+[\d: ]{5,10}(?:\s+WIB)?)?\s+(.+?)\s+([+-]?[\d,.]+)\s+[\d,.]+/gi;
      while ((match = appPatternLocal.exec(cleanText)) !== null) {
        ...
        results.push(
          createMappedItem(
            ...
            amount,
            type,
            forcedSource || "Mobile App",
          ),
        );
      }

      // 6. Generic last resort
      const genericPatternLocal =
        /(\d{1,2}[-\/]\d{1,2}[-\/]\d{2,4})\s+(.+?)\s+(-?[\d,.]+)/g;
      ...
      while ((match = genericPattern.exec(cleanText)) !== null) {
        ...
        results.push(
          createMappedItem(index++, date, desc, Math.abs(amt), type),
        );
      }
      ...
      finalResults.forEach((r) => {
        ...
        if (!seen.has(key)) {
          ...
          r.description.includes("Transaksi BCA") ||
          r.description.includes("Mutasi")
        ) {
      ...
      if (uniqueResults.length > 0) {
        showNotification(
          `Berhasil mendeteksi ${uniqueResults.length} transaksi dari teks!`,
          "success",
        );
      } else {
        showNotification(
          "Tidak ada transaksi yang terdeteksi. Silakan periksa hasil scan teks di bawah.",
          "warning",
        );
        setShowRawPaste(true);
      }
      ...
      showNotification(
        "Terjadi kesalahan saat mengolah teks. Silakan periksa formatnya.",
        "error",
      );
    }
  };
  ...
  const createMappedItem = (index, date, description, amount, type, source) => {
    ...
    return {
      ...
      payment_method:
        finalType === "transfer" ? "Internal / Balance" : "Transfer",
    ...
    reader.onload = (evt) => {
      ...
      for (let i = 0; i < Math.min(rawData.length, 15); i++) {
        ...
        if (
          ...
          row.some(
            (cell) =>
              cell &&
              headerKeywords.some((kw) =>
                cell.toString().toLowerCase().includes(kw),
              ),
          )
        ) {
          headerRowIndex = i;
      ...
      const mappedData = data
        .map((row, index) => {
          const getVal = (possibleKeys) => {
            ...
            for (const pKey of possibleKeys) {
              const match = rowKeys.find(
                (rk) => rk.trim().toLowerCase() === pKey.toLowerCase(),
              );
          ...
          const rawDesc =
            getVal(["Description", "Keterangan", "Uraian", "Notes"]) || "";
          ...
          if (rawDb && sanitizeAmount(rawDb) > 0) {
            ...
            else if (sign && sign.toString().toUpperCase() === "DB")
              type = "expense";
            else if (amt < 0) type = "expense";
            else if (rawDesc.toUpperCase().includes("CR"))
          ...
          const finalAccount =
            getVal(["Account", "Akun"]) || importTargetAccount;
          ...
          return {
            ...
            description: rawDesc,
            payment_method:
              getVal(["Payment Method", "Metode Pembayaran"]) ||
              (finalType === "transfer"
                ? "Internal / Balance"
                : type === "income"
                  ? "Transfer"
  ...
  const handleImport = async () => {
    ...
    if (toImport.length === 0) {
      showNotification(
        "Pilih setidaknya satu transaksi untuk diimport.",
        "warning",
      );
      return;
    }
    ...
    try {
      ...
      if (successCount > 0) {
        showNotification(
          `Berhasil mengimpor ${successCount} transaksi!${failCount > 0 ? ` (${failCount} gagal)` : ""}`,
          "success",
        );
        ...
        showNotification(
          "Gagal mengimpor transaksi. Coba lagi nanti atau hubungi support.",
          "error",
        );
      }
    } catch (error) {
    ...
    <div className="space-y-6">
      <div className="bg-slate-50 dark:bg-slate-900/50 p-6 rounded-2xl border border-slate-200 dark:border-slate-800">
        <div className="flex flex-col md:flex-row gap-6 items-center">
              ...
              {ACCOUNTS.map((acc) => (
                <button
                  ...
                  className={`px-3 py-2 text-xs font-bold rounded-xl border transition-all ${
                    importTargetAccount === acc
                      ? "bg-finance-primary dark:bg-blue-600 text-white border-finance-primary dark:border-blue-600 shadow-lg shadow-finance-primary/20 dark:shadow-blue-900/20"
          ...
          <div className="w-px h-20 bg-slate-200 dark:bg-slate-800 hidden md:block"></div>

          <div className="flex-1 w-full">
            <label className="block text-sm font-bold text-slate-700 dark:text-slate-300 mb-2">
              2. Upload File (PDF/Excel)
            </label>
            <div className="flex flex-col gap-2">
              <div
                className="p-6 border-2 border-dashed border-slate-200 dark:border-slate-800 rounded-2xl flex flex-col items-center justify-center bg-white dark:bg-slate-900 hover:bg-slate-50 dark:hover:bg-slate-800 transition-colors cursor-pointer group"
                onClick={() => document.getElementById("excel-upload").click()}
              >
                <div className="flex gap-2 mb-2">
                  <FileUp
                    size={24}
                    className="text-slate-400 group-hover:text-finance-primary transition-colors"
                  />
                  <FileText
                    size={24}
                ...
                <div className="text-center">
                  <span className="text-xs font-semibold text-slate-500 dark:text-slate-400 block">
                    {fileName || "Klik untuk upload PDF, Foto, atau Excel"}
                  </span>
                  {isLoading && ocrProgress > 0 && (
                    <div className="mt-2 w-48 bg-slate-100 dark:bg-slate-800 rounded-full h-1.5 overflow-hidden mx-auto">
                      <div
              ...
              <button
                ...
                className="flex items-center justify-center gap-2 py-2 px-4 rounded-xl text-[10px] font-black uppercase tracking-widest text-slate-400 hover:text-slate-600 dark:hover:text-slate-200 hover:bg-slate-100 dark:hover:bg-slate-800 transition-all"
              >
                <ClipboardList size={14} />
                {showRawPaste
                  ? "Tutup Pilihan Tempel"
        ...
        {showRawPaste && (
          <div className="mt-6 pt-6 border-t border-slate-200 dark:border-slate-800 animate-slide-up">
            <div className="flex items-center gap-2 mb-3">
              <h6 className="text-xs font-black uppercase text-slate-400 dark:text-slate-500 flex items-center gap-2">
                <ClipboardList size={14} /> Tempel Teks Mutasi BNI/BCA
              </h6>
              <span className="text-[10px] bg-rose-50 dark:bg-rose-900/20 text-rose-500 dark:text-rose-400 px-1.5 py-0.5 rounded font-black border border-rose-100 dark:border-rose-800 uppercase tracking-tighter">
                Solusi PDF Password
              </span>
            </div>
            <p className="text-[10px] text-slate-500 dark:text-slate-400 mb-3 italic">
              Buka PDF-nya, masukkan password, lalu{" "}
              <b>pilih/blok semua teks tabel mutasi</b>, copy, dan tempel di
              kotak bawah ini.
            </p>
            <textarea
              className="w-full min-h-[250px] p-4 text-xs font-mono bg-white dark:bg-slate-800 border border-slate-200 dark:border-slate-700 rounded-xl focus:ring-2 focus:ring-finance-primary/20 dark:focus:ring-blue-600/20 focus:border-finance-primary dark:focus:border-blue-600 outline-none transition-all placeholder:text-slate-300 dark:placeholder:text-slate-600 dark:text-slate-200 resize-y"
      ...
      {previewData.length > 0 && (
        <div className="space-y-4 animate-fade-in">
          <div className="flex flex-col sm:flex-row items-center justify-between gap-4 bg-slate-50 dark:bg-slate-900/50 p-4 rounded-xl border border-slate-200 dark:border-slate-800">
            <div className="flex items-center gap-3">
              <div className="bg-white dark:bg-slate-800 p-2 rounded-lg border border-slate-200 dark:border-slate-700 shadow-sm">
                <Table
                  size={20}
                  className="text-finance-primary dark:text-blue-500"
                />
              </div>
              <div>
                ...
                <p className="text-xs text-slate-500 dark:text-slate-400">
                  {selectedIds.size} terpilih, duplikat dilewati.
                </p>
              </div>
            </div>
            <div className="flex gap-2">
              <Button
                ...
                variant="ghost"
                size="sm"
                className="text-xs font-bold text-slate-500 dark:text-slate-400 dark:hover:bg-slate-800"
              >
              ...
              <Button
                ...
                className="flex items-center gap-2 px-6 shadow-lg shadow-finance-primary/20 dark:shadow-blue-900/20"
              >
                {isLoading ? (
          ...
          <div className="overflow-x-auto border border-slate-200 dark:border-slate-800 rounded-xl overflow-hidden shadow-sm max-h-[600px] overflow-y-auto custom-scrollbar">
            <table className="w-full text-sm text-left border-collapse">
              <thead className="bg-slate-50 dark:bg-slate-800 text-slate-500 dark:text-slate-400 border-b border-slate-200 dark:border-slate-700 sticky top-0 z-20">
                <tr>
                  <th className="py-3 px-4 w-10">
                    <input
                      ...
                      className="rounded border-slate-300 dark:border-slate-700 bg-white dark:bg-slate-800 text-finance-primary focus:ring-finance-primary"
                    />
                  </th>
                  <th className="py-3 px-4 font-bold uppercase text-[9px] tracking-widest">
                    Tgl
                  </th>
                  <th className="py-3 px-4 font-bold uppercase text-[9px] tracking-widest">
                    Type
                  ...
                  <th className="py-3 px-4 font-bold uppercase text-[9px] tracking-widest">
                    Kategori / Akun Tujuan
                  </th>
                  <th className="py-3 px-4 font-bold uppercase text-[9px] tracking-widest text-right">
                    Nominal
                  </th>
                  <th className="py-3 px-4 font-bold uppercase text-[9px] tracking-widest">
                    Keterangan
              ...
              <tbody className="divide-y divide-slate-100 dark:divide-slate-800 bg-white dark:bg-slate-900">
                {previewData.map((item) => (
                  <tr
                    key={item.id}
                    className={`group hover:bg-slate-50/50 dark:hover:bg-slate-800/30 transition-colors ${item.isDuplicate ? "bg-rose-50/20 dark:bg-rose-900/10" : ""}`}
                  >
                    ...
                    <td className="py-3 px-4 text-slate-600 dark:text-slate-400 whitespace-nowrap tabular-nums font-medium">
                      {item.date}
                    </td>
                    <td className="py-3 px-4">
                      <Badge
                        ...
                        className="text-[8px] py-0.5 px-1.5 uppercase font-black tracking-tighter"
                      >
                        {item.type}
                      </Badge>
                    </td>
                    <td className="py-3 px-4">
                      <div className="flex flex-col gap-1">
                        ...
                        {item.type === "transfer" ? (
                          <div className="flex items-center gap-2">
                            ...
                            <select
                              ...
                              className="text-xs font-bold text-indigo-600 dark:text-indigo-400 bg-indigo-50 dark:bg-indigo-900/30 border-none rounded py-0.5 px-1.5 focus:ring-1 focus:ring-indigo-300 dark:focus:ring-indigo-800"
                            >
                          ...
                          <div className="flex items-center gap-1.5">
                            ...
                            {item.isAuto && (
                              <span className="text-[8px] bg-emerald-50 dark:bg-emerald-900/30 text-emerald-600 dark:text-emerald-400 px-1 py-0.5 rounded font-black uppercase tracking-tighter border border-emerald-100 dark:border-emerald-800">
                                Auto
                              </span>
                            )}
                          </div>
                        ...
                        {item.isDuplicate && (
                          <span className="text-[8px] text-rose-500 dark:text-rose-400 font-black uppercase flex items-center gap-1">
                    ...
                    <td
                      className={`py-3 px-4 text-right font-black tabular-nums ${item.type === "income" ? "text-emerald-600 dark:text-emerald-400" : item.type === "transfer" ? "text-indigo-600 dark:text-indigo-400" : "text-slate-900 dark:text-slate-100"}`}
                    >
                      {item.type === "expense" ? "-" : "+"}
                    ...
                    <td
                      className="py-3 px-4 text-slate-500 dark:text-slate-400 text-[11px] leading-tight max-w-[250px] truncate"
      ...
      <div className="bg-blue-50/50 dark:bg-blue-900/10 p-4 rounded-xl flex gap-3 items-start border border-blue-100 dark:border-blue-900/30">
        <AlertCircle
          className="text-blue-500 dark:text-blue-400 mt-0.5 flex-shrink-0"
          size={18}
        />
        <div className="text-[10px] text-blue-700 dark:text-blue-300 leading-relaxed uppercase tracking-wider font-bold">
          <p className="mb-1">Tips Import Pintar:</p>
          <p className="text-blue-600/80 dark:text-blue-400/80 normal-case font-medium">
            Sistem otomatis mendeteksi <b>Flip, BI-FAST,</b> dan transfer antar
            rekening berdasarkan catatan (description). Jika deteksi salah,
            Bapak bisa ubah langsung di tabel preview sebelum simpan.
          </p>
        </div>
      </div>
    </div>
