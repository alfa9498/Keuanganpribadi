...
// Sidebar Item Component
// Sidebar Item Component
const SidebarItem = ({ item, activeTab, onTabChange, isOpen, toggleSidebar, isMinimized }) => {
    ...
    return (
        ...
        >
            <button
                ...
                className={`flex items-center w-full px-4 py-3 rounded-lg transition-all duration-200 group justify-between ${isActive && !item.children
            ...
            >
                <div className={`flex items-center ${isMinimized ? 'justify-center w-full' : ''}`}>
                    <Icon size={20} className={`${!isMinimized ? 'mr-3' : ''} ${isActive && !item.children ? 'text-white' : 'text-slate-500 group-hover:text-white'} transition-colors`} />
                ...
                <div className={!isMinimized
                    ...
                    {item.children.map(child => (
                        <button
                            ...
                            className={`flex items-center w-full px-4 py-2 rounded-lg text-sm transition-colors duration-200 ${activeTab === child.id
...
export const SidebarOrganism = ({ activeTab, onTabChange, isOpen, toggleSidebar, onLogout, user, isMinimized, setIsMinimized }) => {
    ...
    return (
        <>
            ...
            {/* Sidebar */}
            <aside
                className={`fixed top-0 left-0 z-30 h-screen bg-slate-900 text-white transition-all duration-300 ease-in-out lg:translate-x-0 lg:static ${isOpen ? 'translate-x-0' : '-translate-x-full'
                    } ${isMinimized ? 'w-20' : 'w-64'}`}
            >
                {/* Logo */}
                <div className={`flex items-center p-6 border-b border-slate-800 transition-all duration-300 relative ${isMinimized ? 'justify-center' : 'justify-between'}`}>
                    {!isMinimized ? (
                        <h1 className="text-2xl font-bold tracking-tight text-white whitespace-nowrap overflow-hidden transition-all duration-300">
                            Finance<span className="text-finance-primary uppercase italic text-xl ml-1">App</span>
                        </h1>
                    ) : (
                ...
                {/* Navigation */}
                <nav className={`p-4 space-y-1 transition-all duration-300 ${isMinimized ? 'px-2' : ''}`}>
                    {!isMinimized && (
                ...
                {/* Bottom Actions */}
                <div className={`absolute bottom-0 left-0 w-full p-4 border-t border-slate-800 transition-all duration-300 ${isMinimized ? 'px-2' : ''}`}>
                    ...
                    <div className="relative mb-3" ref={userMenuRef}>
                        ...
                        >
                            <span className={`h-9 w-9 rounded-full bg-gradient-to-tr from-slate-700 to-slate-600 flex items-center justify-center overflow-hidden flex-shrink-0 border-2 border-slate-700 shadow-inner`}>
                                <img
                                    src={user?.gender === 'female' ? '/female-avatar.png' : '/male-avatar.png'}
                                    alt="User"
                            ...
                            {!isMinimized && (
                                <>
                                    <div className="ml-3 flex-1 text-left overflow-hidden">
                                        ...
                                        <p className="text-[10px] text-slate-500 truncate uppercase font-medium">{user?.role || 'Personal Account'}</p>
                                    </div>
                    ...
                    >
                        <LogOut size={20} className={`${!isMinimized ? 'mr-3' : ''} group-hover:scale-110 transition-transform`} />
                        {!isMinimized && <span className="font-bold text-sm">Sign Out</span>}
                    </button>
                </div>
            </aside>
        </>
