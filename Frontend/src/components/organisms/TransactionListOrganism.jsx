...
export const TransactionListOrganism = ({ user }) => {
    const [isImportOpen, setIsImportOpen] = useState(false);
    const [isAddOpen, setIsAddOpen] = useState(false);
    const [activeListTab, setActiveListTab] = useState('all'); // all, income, expense
    const [filterRange, setFilterRange] = useState('30D'); // Default 30 Days like Kibana/Logs
    ...
    return (
        <div className="w-full max-w-[1600px] p-4 space-y-4 animate-fade-in">
            <div className="flex flex-col xl:flex-row justify-between items-center gap-4 bg-white p-4 rounded-2xl border border-slate-100 shadow-sm">
                ...
                <div className="flex bg-slate-100 p-1 rounded-xl w-full xl:w-auto overflow-x-auto">
                    <button
                        ...
                        className={`px-6 py-2 text-sm font-bold rounded-lg transition-all whitespace-nowrap ${activeListTab === 'all'
                    ...
                    <button
                        ...
                        className={`px-6 py-2 text-sm font-bold rounded-lg transition-all whitespace-nowrap ${activeListTab === 'income'
                    ...
                    <button
                        ...
                        className={`px-6 py-2 text-sm font-bold rounded-lg transition-all whitespace-nowrap ${activeListTab === 'expense'
            ...
            <RecentTransactionsOrganism
                key={`${activeListTab}-${refreshKey}-${filterRange}`} // Force remount on tab change or add success
                ...
                title={
                    activeListTab === 'income' ? 'Daftar Pemasukan' :
                        activeListTab === 'expense' ? 'Daftar Pengeluaran' :
                            'Semua Transaksi'
                }
            />
