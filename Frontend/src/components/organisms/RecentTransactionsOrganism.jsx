...
import { Search, Loader2, AlertTriangle, Trash2, Edit, ChevronUp, ChevronDown, ArrowUpDown, ChevronLeft, ChevronRight, Inbox, TrendingUp, TrendingDown } from 'lucide-react';
...
export const RecentTransactionsOrganism = ({ transactions, onViewAll, user, onRefresh, fixedType = null, title = "Transaksi Terakhir", filterCategory: externalCategory, filterRange = 'ALL', onRangeChange }) => {
    ...
    const activeCategory = externalCategory !== undefined ? externalCategory : selectedCategory;
    ...
    const calculateDateRange = (range) => {
        ...
        return { startDate: formatDate(startDate), endDate: formatDate(endDate || now) };
    };
    ...
    useEffect(() => {
        fetchTransactions();
    }, [user?.id, searchTerm, activeCategory, selectedAccount, sortConfig, currentPage, rowsPerPage, filterRange]);
    ...
    useEffect(() => {
        ...
        return () => window.removeEventListener('transaction_updated', handleRealtimeUpdate);
    ...
    const confirmDelete = async () => {
        ...
        try {
            ...
            if (response.ok) {
                ...
                showNotification(`Gagal menghapus transaksi: ${result.message || 'Unknown Error'}`, 'error');
            }
        } catch (error) {
        ...
        <div className="bg-white rounded-3xl shadow-xl shadow-slate-200/50 border border-slate-100 overflow-hidden">
            ...
            <div className="p-6 pb-4 flex flex-col sm:flex-row justify-between items-center gap-4">
                <div>
                    ...
                    <p className="text-sm text-slate-500">Mengelola rekaman terbaru Anda</p>
                </div>
                ...
                <div className="flex flex-wrap items-center gap-3 w-full sm:w-auto">
                    ...
                    <div className="relative w-full sm:w-64 group">
                        <Search size={18} className="absolute left-3 top-1/2 -translate-y-1/2 text-slate-400 group-focus-within:text-finance-primary transition-colors" />
                        <input
                            type="text"
                    ...
                    <thead className="sticky top-0 z-10 bg-white shadow-sm">
                        <tr className="border-y border-slate-100/80 text-slate-500 text-[11px] uppercase tracking-wider font-semibold">
                            {[
                                ...
                                <th
                                    key={col.key}
                                    className={`py-3 px-4 cursor-pointer hover:bg-slate-50 transition-colors select-none ${col.align === 'right' ? 'text-right' : ''}`}
                                    onClick={() => requestSort(col.key)}
                                >
                                    <div className={`flex items-center gap-1.5 ${col.align === 'right' ? 'justify-end' : ''}`}>
                                        {col.label}
                                        {sortConfig.key === col.key ? (
                                            sortConfig.direction === 'asc' ? <ChevronUp size={12} className="text-finance-primary" /> : <ChevronDown size={12} className="text-finance-primary" />
                                        ) : (
                                            <ArrowUpDown size={12} className="text-slate-300 group-hover:text-slate-400 transition-colors" />
                                        )}
                                    </div>
                                </th>
                            ))}
                    ...
                    <tbody className="divide-y divide-slate-50 relative">
                        {isLoading && (
                            <tr className="absolute inset-0 bg-white/50 z-10 flex items-center justify-center">
                                <td colSpan="8" className="py-20 text-center">
                                    <Loader2 className="animate-spin text-finance-primary mx-auto" size={40} />
                                </td>
                            </tr>
                        )}
                        {backendTransactions.length > 0 ? (
                            backendTransactions.map(tx => {
                                return (
                                    <tr key={tx.id} className="group hover:bg-indigo-50/30 transition-all duration-200">
                                        <td className="py-4 px-4 text-slate-700 whitespace-nowrap text-sm">
                                            <span className="font-medium">{new Date(tx.date).toLocaleDateString('id-ID', { day: '2-digit', month: 'short' })}</span>
                                            <span className="text-slate-400 ml-1 text-[11px]">{new Date(tx.date).getFullYear()}</span>
                                        </td>
                                        <td className="py-4 px-4">
                                            <div className="flex items-center gap-2">
                                                ...
                                                <div className="flex flex-col">
                                                    <span className="text-sm font-bold text-slate-800 leading-tight">{getCategoryGroup(tx.category) || tx.category}</span>
                                                    {getCategoryGroup(tx.category) && getCategoryGroup(tx.category) !== tx.category && (
                                                        <span className="text-[10px] text-slate-400 font-medium leading-tight">{tx.category}</span>
                                                    )}
                                                </div>
                                            </div>
                                        </td>
                                        <td className="py-4 px-4 text-slate-600 max-w-xs text-sm truncate group-hover:text-slate-900 transition-colors" title={tx.description}>
                                            {tx.description || <span className="text-slate-300 italic">No notes</span>}
                                        </td>
                                        <td className="py-4 px-4 text-slate-500 text-xs font-medium">
                                            <span className="bg-slate-100 px-2 py-1 rounded-md">{tx.payment_method}</span>
                                        </td>
                                        <td className="py-4 px-4 text-slate-600 font-bold text-xs">
                                            {tx.account}
                                        </td>
                                        <td className="py-4 px-4">
                                            <Badge variant={tx.status === 'done' ? 'info' : 'warning'} className="rounded-md font-bold">
                                                {tx.status === 'done' ? 'DONE' : 'PENDING'}
                                            </Badge>
                                        </td>
                                        <td className={`py-4 px-4 text-right text-sm font-medium tabular-nums ${tx.type === 'income' ? 'text-emerald-600' : 'text-rose-600'
                                            }`}>
            ...
            <div className="px-6 py-4 bg-slate-50/30 border-t border-slate-100 flex flex-col sm:flex-row justify-between items-center gap-4">
                <div className="text-xs text-slate-500 font-medium">
                    Menampilkan <span className="text-slate-800">{(currentPage - 1) * rowsPerPage + 1}</span> sampai <span className="text-slate-800">{Math.min(currentPage * rowsPerPage, totalItems)}</span> dari <span className="text-slate-800">{totalItems}</span> rekaman
                </div>
                ...
                <div className="flex items-center gap-2">
                    ...
                    {[...Array(totalPages)].map((_, i) => (
                        <button
                            ...
                            className={`w-8 h-8 text-[11px] font-bold rounded-lg transition-all ${currentPage === i + 1
            ...
            >
                <div className="flex flex-col items-center text-center p-4">
                    ...
                    <h4 className="text-xl font-bold text-slate-800 mb-2">Apakah Anda yakin?</h4>
                    <p className="text-slate-500 mb-6">
                        ...
                        <span className="font-semibold text-slate-700 block mt-1">
                            {selectedTx?.description} ({selectedTx ? formatCurrency(selectedTx.amount) : ''})
                        </span>
                    </p>
