...
export const CategoryFilter = ({ currentCategory, onCategoryChange }) => {
    ...
    const incomeCategories = ["Gaji", "Bonus", "Hadiah", "Penjualan", "Investasi", "Bunga Bank", "Saldo Awal", "Piutang", "Hutang", "Lainnya"];
        ...
        <div className="flex flex-wrap items-center gap-2">
            <div className="relative group">
                ...
                >
                    ...
                    <optgroup label="Pemasukan" className="bg-slate-50 text-emerald-600 font-bold py-2 uppercase tracking-tighter text-[10px]">
                        {incomeCategories.map(cat => (
                            <option key={`in-${cat}`} value={cat} className="text-slate-700 font-medium">{cat}</option>
                        ))}
                    </optgroup>

                    <optgroup label="Pengeluaran" className="bg-slate-50 text-amber-600 font-bold py-2 uppercase tracking-tighter text-[10px]">
                        {Object.keys(expenseCategories).map(group => (
                            <option key={`ex-${group}`} value={group} className="text-slate-700 font-medium">{group}</option>
                        ))}
                    </optgroup>
                </select>
