...
exports.getTransactions = (req, res) => {
    const { user_id, startDate, endDate, type, category, account, limit, search, sortBy, sortOrder, page } = req.query;
    ...
    // Search filter
    if (search) {
        query += " AND (description LIKE ? OR category LIKE ? OR account LIKE ? OR payment_method LIKE ?)";
    ...
    // Sort
    const validSortKeys = ['date', 'category', 'description', 'payment_method', 'account', 'status', 'amount'];
    ...
    if (search) {
        countQuery += " AND (description LIKE ? OR category LIKE ? OR account LIKE ? OR payment_method LIKE ?)";
    ...
    db.query(countQuery, countParams, (err, countResult) => {
        if (err) {
            ...
            return res.status(500).json({ message: "Gagal mengambil total data transaksi" });
        }
...
exports.updateTransaction = (req, res) => {
    const { id } = req.params;
    const { date, type, category, amount, description, payment_method, account, to_account, status } = req.body;
    ...
    console.log(`[Update] Transaction ${id}:`, { date, type, category, numAmount, description, payment_method, account, to_account, status });
    ...
    db.query(query, [date, type, category, numAmount, description, val_payment, val_account, val_to_account, val_status, id], (err, result) => {
        if (err) {
        ...
        console.log(`[Update] Success: Transaction ${id} updated (Affected rows: ${result.affectedRows})`);
        ...
        if (req.body.user_id) {
            ...
            notificationController.createNotification(req.body.user_id, 'transaction_updated', notifTitle, notifMessage);
        }

        // BROADCAST REAL-TIME UPDATE
        if (global.io) {
            console.log(`📢 Broadcasting transaction_updated event for ID: ${id}`);
            global.io.emit('transaction_updated', { type: 'update', data: { id, user_id: req.body.user_id } });
        }
...
exports.deleteTransaction = (req, res) => {
    ...
    const getQuery = "SELECT user_id, category, amount FROM transactions WHERE id = ?";
    ...
    db.query(getQuery, [id], (err, transactions) => {
        ...
        db.query(deleteQuery, [id], (err, result) => {
            ...
            notificationController.createNotification(transaction.user_id, 'transaction_deleted', notifTitle, notifMessage);

            // BROADCAST REAL-TIME UPDATE
            if (global.io) {
                console.log(`📢 Broadcasting transaction_updated event for deleted ID: ${id}`);
                global.io.emit('transaction_updated', { type: 'delete', data: { id, user_id: transaction.user_id } });
            }
