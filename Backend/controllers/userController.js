...
exports.register = (req, res) => {
  ...
  const query =
    "INSERT INTO users (full_name, email, phone, password, gender) VALUES (?, ?, ?, ?, ?)";
  db.query(
    query,
    [fullName, email, phone, password, gender || "male"],
    (err, result) => {
      if (err) {
...
exports.requestPasswordReset = (req, res) => {
  ...
  if (!email || !phone) {
    return res
      .status(400)
      .json({ message: "Email dan No. Telepon wajib diisi!" });
  }
  ...
  db.query(query, [email, phone], async (err, results) => {
    ...
    if (results.length === 0) {
      return res
        .status(404)
        .json({ message: "User tidak ditemukan atau data tidak cocok!" });
    }
    ...
    db.query(insertQuery, [user.id, otp, expiresAt], async (insertErr) => {
      ...
      if (user.telegram_chat_id) {
        ...
        sendTelegram(message, user.telegram_chat_id);
      }

      res.json({
        message: "OTP telah dikirim ke Email dan Platform Chat (WA/Telegram)",
      });
    });
  });
};
...
exports.generateTelegramLink = (req, res) => {
  ...
  try {
    ...
    if (!botUsername) {
      return res.status(500).json({
        message:
          "Aplikasi belum dikonfigurasi: TELEGRAM_BOT_USERNAME belum diset di server.",
      });
    }
    res.status(200).json({
      linkToken: linkToken,
      ...
      verificationCode: null, // Will be generated by separate endpoint
    });
  } catch (err) {
    console.error(err);
...
// Generate simple 6-digit verification code for manual linking
exports.generateVerificationCode = (req, res) => {
  ...
  try {
    ...
    // Store in telegramBotService (we'll need to export a function for this)
    const telegramBotService = require("../services/telegramBotService");
    telegramBotService.storeVerificationCode(code, {
      userId: userId,
      fullName: decoded.fullName,
